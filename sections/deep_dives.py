...
# ISO-2 → ISO-3 mapping for Europe (good coverage for Eurostat)
_ISO2_TO_ISO3 = {
...
def render_deep_dives(
    ...
    with st.sidebar:
        ...
        if all_countries:
            country = st.selectbox("Primary country", all_countries, index=0, key="dd_primary")
            ...
            country, compare_countries = None, []
    ...
    if selected_countries or y0 is not None:
        ...
        years_txt = f"{y0:.0f} to {y1:.0f}" if (y0 is not None and y1 is not None) else "All years"
        st.markdown(f'<div class="chip-row">{chips}<span class="pink-chip">Years: {years_txt}</span></div>', unsafe_allow_html=True)

    # Tabs (including the interactive & map tabs)
    ...
    with t1:
        ...
        if mort_sub is not None and not mort_sub.empty and {"mortality_rate", "year", "country"}.issubset(mort_sub.columns):
            ...
            line = (
                ...
                .encode(
                    x=alt.X("year:Q", title="Year"),
                    y=alt.Y("mortality_rate:Q", title="Deaths per 100,000 (age < 50 preferred)"),
                    color=color,
    ...
    with t2:
        ...
        if can_use_screening_age:
            ...
            if not exam_f.empty and {"age_group", "exam_rate", "country", "year"}.issubset(exam_f.columns):
                e = exam_f.copy()
                ...
                if not e.empty:
                    last_y = e["year"].dropna()
                    if last_y.empty:
                        st.info("No year information available in the exam dataset for the current selection.")
                    else:
                        last_y = int(last_y.max())
                        ...
                        if not comp.empty:
                            ...
                            st.info("Insufficient data to compare 40–49 vs 50–69 in the exam dataset.")
                else:
    ...
    with t3:
        ...
        if not mort_f.empty and {"mortality_rate", "year", "country", "age"}.issubset(mort_f.columns):
            ...
            if not under.empty and not total.empty:
                ...
                merged = merged[(merged["mort_total"].notna()) & (merged["mort_total"] > 0)]
                if not merged.empty:
                    ...
                    st.info("Not enough paired under-50 and TOTAL mortality rows to compute a share.")
            else:
                st.info("Need both under-50 and TOTAL mortality rows to compute the share.")
        else:
    ...
    with t4:
        ...
        if not exam_f.empty and {"country", "year", "age_group", "income_quintile", "exam_rate"}.issubset(exam_f.columns):
            ...
            if e["year"].dropna().empty:
                ...
                if e_last.empty:
                    st.info("No data rows in the latest survey year for the selection.")
                else:
                    age_order = sorted(e_last["age_group"].astype(str).unique().tolist(), key=lambda s: (len(s), s))
                    quint_order = ["Q1", "Q2", "Q3", "Q4", "Q5"]

                    options = selected_countries or e_last["country"].dropna().unique().tolist()
                    if not options:
                        st.info("No countries available for the heatmap.")
                    else:
                        c_heat = st.selectbox("Select country for heatmap", options, key="dd_heat_country")
    ...
    with t5:
        st.markdown("#### Interactive screening trend (hover, select in legend, brush to zoom)")

        if not scr_f.empty and {"screening_rate", "year", "country"}.issubset(scr_f.columns):
            ts = scr_f.groupby(["country", "year"], as_index=False)["screening_rate"].mean().sort_values(["country", "year"])

            # Altair v4-friendly selections
            ...
            lines = base.mark_line().add_selection(select_country)
            points = base.mark_point(size=40, filled=True).add_selection(hover).transform_filter(select_country | (select_country.empty()))
            rules = alt.Chart(ts).mark_rule(color="lightgray").encode(x="year:Q").transform_filter(hover)
            text = base.mark_text(align="left", dx=5, dy=-5).encode(text=alt.condition(hover, alt.datum.screening_rate, alt.value("")))

            zoomed = (lines + points + rules + text).add_selection(brush).transform_filter(brush)
        ...
        if not scr_f.empty and {"screening_rate", "year", "country"}.issubset(scr_f.columns):
            latest_years = scr_f["year"].dropna()
            if latest_years.empty:
                st.info("No year information available for the latest cross-section.")
            else:
                latest_y = int(latest_years.max())
                latest = (
                ...
                hover_bar = alt.selection_single(on="mouseover", empty="all", fields=["country"])
                bars = (
                    alt.Chart(latest)
                    .mark_bar()
                    .encode(
    ...
    with t6:
        ...
        # Build from the *global* tables (not only selected countries) but still respect year slider,
        # so the map shows the full European context for the chosen period.
        scr_all = _filter_years(df_screening, y0, y1)
        mort_all = _filter_years(df_mortality, y0, y1)
        exam_all = _filter_years(df_exam_income, y0, y1)

        if map_metric.startswith("Screening"):
            if not scr_all.empty and {"screening_rate", "year", "country"}.issubset(scr_all.columns):
                y_series = scr_all["year"].dropna()
            ...
            if ei_sub_all is not None and not ei_sub_all.empty and {"income_quintile","exam_rate","year","country"}.issubset(ei_sub_all.columns):
                y_series = ei_sub_all["year"].dropna()
                if not y_series.empty:
                    y = int(y_series.max())
